    ...
    <section class="min-h-screen bg-gray-50 dark:bg-gray-950 py-16 px-4">
      <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-wrap gap-2 justify-center mb-10" role="group" aria-label="Filtruj według kategorii">
          <button (click)="setCategory(null)" class="px-5 py-2 rounded-full text-sm font-medium transition-colors" [class.bg-[#1E3A5F]]="!activeCategory()" [class.text-white]="!activeCategory()" [class.bg-white]="activeCategory() !== null" [class.dark:bg-gray-800]="activeCategory() !== null" [class.text-gray-700]="activeCategory() !== null" [class.dark:text-gray-300]="activeCategory() !== null" [class.hover:bg-gray-100]="activeCategory() !== null" [attr.aria-pressed]="!activeCategory()">
            Wszystkie ({{ items.length }})
          </button>
          @for (cat of categories; track cat) {
            <button (click)="setCategory(cat)" class="px-5 py-2 rounded-full text-sm font-medium transition-colors" [class.bg-[#1E3A5F]]="activeCategory() === cat" [class.text-white]="activeCategory() === cat" [class.bg-white]="activeCategory() !== cat" [class.dark:bg-gray-800]="activeCategory() !== cat" [class.text-gray-700]="activeCategory() !== cat" [class.dark:text-gray-300]="activeCategory() !== cat" [attr.aria-pressed]="activeCategory() === cat">
              {{ cat }} ({{ countFor(cat) }})
            </button>
          }
        </div>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          @for (item of filteredItems(); track item.id) {
            <div class="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              ...
              />
              <div class="p-4">
                <span class="inline-block bg-blue-100 dark:bg-blue-900/30 text-[#1E3A5F] dark:text-blue-400 text-xs font-semibold px-2 py-1 rounded-full mb-2">{{ item.category }}</span>
